 ...
 */
function LinePaint(gameRef, gameClass) {
    ...
    var winAnim= {
        ...
        "winTxtDuration": [.5,  .6,   .8,   1,   1.1, 2,   5,   6,   7  ,10],
    ...
    var winAnimB= {
        ...
        "winTxtDuration": [.2,  .2,   .2,   .3,   .5, .5,   2,   3,   4  ,5],
        "scaleFactor":    [1.1,  1.15 ,1.2, 1.5, 1.7, 1.8, 2,   2.2, 2.3,   2.4],
        "winTxtColors": ["#ffffff", "#ffffff", "#ffffff", "#ffffff", "#ffffff", "#ffffff", "#ffffff", "ffffff", "#ffffff", "#ffffff"],
        "winSndEffect": ["", "", "", "", "", "", "", "", "", ""],
        "accuSnd": "winBg"
    ...
    function showBonusWin_(amt,amtCash){
        ...
        stepInc_= 2;
        winManager_.setWinAmountBonus(amtCash);
    ...
    function showLine_() {
        ...
        step_=1;
        totalDuration_=0;
        stepInc_= winAnim.scaleFactor[findWinRange_(winManager_.getLineWinAmt())]/wins.length;
    ...
    function delayedAnimation_(smb,a){
        var add=false;
        if (spinManager_.getAnimationMap(smb.reel[a],smb.icon[a])==undefined || spinManager_.getAnimationMap(smb.reel[a],smb.icon[a]).animations==null || ReelConfig.keepShowingAnimation==true) //avoid to add the same animation more then once
        {

            if(spinManager_.getAnimationMap(smb.reel[a],smb.icon[a])!=undefined && spinManager_.getAnimationMap(smb.reel[a],smb.icon[a]).animations!=null){
    ...
    function resetCentralWin_(){
        ...
        if (bgStopped_){
            soundManager_.resumeBgSound("bgSlot");
            bgStopped_=false;
        }
        stage_=0;
        totalDuration_=0;
    ...
    function changeCentralWinStage_(bonus){
        ...
        if (stage_>0){
            ...
            soundManager_.stopBgSound("bgSlot");
            bgStopped_=true;
        }
        stage_++;
    ...
    function startNewObject_(){
        if (Util.getRandomInt(1,100)>50)
            var ob = game_.add.sprite(640, 740, "part-" + Util.getRandomInt(1,stage_));
        else
            var ob = game_.add.sprite(640, 740, "f-part-" + stage_);
        ob.anchor.set(.5, .5);
