/**
 * KiS Framework, Created by Fabry on 12/04/2022.
 */
function MegaWayTumblingWinningsDice(gameRef, gameClass) {
    ...
    function delayedAnimation_(smb,a){
        ...
        if (spinManager_.getAnimationMap(smb.reel[a],smb.icon[a]).animations==null || ReelConfig.keepShowingAnimation==true) //avoid to add the same animation more then once
        {
            ...
            if (game.cache.checkImageKey("anim" + smb.smbNum[a])==true) {
                ...
                if (smb.smbNum[a]==9 ||smb.smbNum[a]==10 ||smb.smbNum[a]==11 ||smb.smbNum[a]==12){
                    ...
                    if (numIcon_==7) {
                        smb.anim[a].scale.x=.65
                        smb.anim[a].scale.y=.65
                    }else if (numIcon_==6){
                        smb.anim[a].scale.x=.80
                        smb.anim[a].scale.y=.80
                    }else if (numIcon_==5){
                        smb.anim[a].scale.x=.95
                        smb.anim[a].scale.y=.95
                    }
                }
            }
            if (smb.smbNum[a]==10){
