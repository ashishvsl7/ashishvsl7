 ...
 */
function NewUIBounceWin(gameRef, gameClass, config, txt){
    ...
    function showWin_(value,start,update,complete,startVal,additionalText,scaleFactor,duration,easing){
        ...
        if (easing!=undefined){
            tweens.push(TweenMax.to(winTxt_,.5,{scaleX:scale_,scaleY:scale_,onComplete:scaleComplete_,repeat:-1,yoyo:true,ease:[easing].easeOut}));
        }else{
            tweens.push(TweenMax.to(winTxt_,.5,{scaleX:scale_,scaleY:scale_,onComplete:scaleComplete_,repeat:-1,yoyo:true}));
        }
    ...
    function showIstantUpdateWin_(value,additionalText){
        logger(" bounce win showIstantUpdateWin_" )
        gcmCalls_("PAID", winManager_.getWinAmtCash());
        //relax
        clientMessageSend("value-win",winManager_.getWinAmtCash());

        additionalPreText_=(additionalText!=null)?(Translate.do(additionalText)+" "):"";
        writeText_(ReelConfig.additionalPreText+additionalPreText_ +  balanceManager_.getCurrencySmb()+ "" + Util.formatNumber(value,Util.getNubersOfDecimal())+ReelConfig.additionalPostText);
        gameClass_.updateBalance(additionalText);
        scaleBack_=winTxt_.scaleX;
        scale_=winTxt_.scaleX+(winTxt_.scaleX*1.2);
        tweens.push(TweenMax.to(winTxt_,.2,{scaleX:scale_,scaleY:scale_,onComplete:showIstantUpdateWinScale_}));
    }

    function showIstantUpdateWinScale_(){
        tweens.push(TweenMax.to(winTxt_,.2,{scaleX:1,scaleY:1}));
    }
    ...
    function scaleComplete2_(){
        ...
        tweens.push(TweenMax.to(winTxt_,.2,{scaleX:winTxt_.scaleX*scaleBack_,scaleY:winTxt_.scaleY*scaleBack_,onComplete:showIstantUpdateWinScale_}));
    ...
    function updatingValue_ (){
        ...
        writeText_(ReelConfig.additionalPreText+ additionalPreText_ +balanceManager_.getCurrencySmb()+ "" + value+ReelConfig.additionalPostText);
