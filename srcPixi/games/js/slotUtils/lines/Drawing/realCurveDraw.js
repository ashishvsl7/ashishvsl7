...
function CCurveDraw(gameRef, gameClass) {
    ...
    function startDraw_(smb, line, amt,points) {
        ...
        if (bezierIconPoints_.length > 0) {
            myEndAlphaPoint_ = bezierIconPoints_.length - myStartEndAlphaPoint_ / 100;
            myIncrAlphaPoint_ = myLineMaxAlpha_ / myStartEndAlphaPoint_;
            oldRandomIconPoints_ = new Point(bezierIconPoints_[0].x, bezierIconPoints_[0].y);
            startDrawLine_(line, lineTweenTime_PointToPoint_, amt);
        }
    };
    ...
    function updateTween_(line, i, pointsL) {
            ...
            if (!isMobile) {
                for (var j = 0; j < 5; j++) {
                    ...
                    drawLineSegment_(container_, oldRandomIconPoints_.x, oldRandomIconPoints_.y + 0.7 * j, bezierIconPoints_[Math.ceil(objTwLine_.amt)].x, bezierIconPoints_[Math.ceil(objTwLine_.amt)].y + 0.7 * j,myAlphaLine_);
                    drawLineSegment_(container_, oldRandomIconPoints_.x, oldRandomIconPoints_.y - 0.7 * j, bezierIconPoints_[Math.ceil(objTwLine_.amt)].x, bezierIconPoints_[Math.ceil(objTwLine_.amt)].y - 0.7 * j,myAlphaLine_);
                }

                //container_.lineStyle(myLineTick_,  lineConfig.line.color, myAlphaLine_);
                drawLineSegment_(container_, oldRandomIconPoints_.x, oldRandomIconPoints_.y, bezierIconPoints_[Math.ceil(objTwLine_.amt)].x, bezierIconPoints_[Math.ceil(objTwLine_.amt)].y,myAlphaLine_);

                //container_.lineStyle(myLineTick_,  lineConfig.line.color, myAlphaLine_);
                oldRandomIconPoints_ = new Point(bezierIconPoints_[Math.ceil(objTwLine_.amt)].x, bezierIconPoints_[Math.ceil(objTwLine_.amt)].y);
    ...
    function drawLineSegment_(container, x1, y1, x2, y2,alpha) {

        var l= new Line(mainGroup_, x1,y1,x2,y2, lineConfig.line.color, alpha)// [, x] [, y] [, x1] [, y1] [, x2] [, y2] [, strokeColor] [, strokeAlpha])
        debugger
        mainGroup_.add(l);
