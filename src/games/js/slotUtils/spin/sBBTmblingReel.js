function  SBBTumblingReel(gameRef,gameClass,_num,num2,swap,len){
    var game_=gameRef;
    ...
    var height_=0;
    var width_=128;
    var characterList=[5,6,7,8,9,12];
    ...
    function setIcon_(index,icon){
        ...
        if (characterList.indexOf(icon)<0 ){
            if (game.cache.checkImageKey("smbBg_"+icon +"_"+numIcon_)){
                bg[index] = game_.add.sprite(pos[index].x + ReelConfig.icon.width / 2, poss,  "smbBg_"+icon +"_"+numIcon_);
            }else{
                bg[index] = game_.add.sprite(pos[index].x + ReelConfig.icon.width / 2, poss, "smbBg_" + numIcon_);
            }
            bg[index].scale.x=1;
            bg[index].scale.y=1;
            ...
            if (isBrandedMw()){
                bg[index] = game_.add.sprite(pos[index].x + ReelConfig.icon.width / 2, poss, "smbFrameBg_" + numIcon_);
                bg[index].scale.x=1;
                bg[index].scale.y=1;
                bg[index].anchor.set(.5,.5);
                group.add(bg[index]);
        ...
        if (ic!=10){
            ...
            img[index]= game_.add.sprite(pos[index].x+ReelConfig.icon.width/2, pos[index].y+height_/2, "icon"+ic+"_"+numIcon_);
        }
        ...
        if (numIcon_>5 && characterList.indexOf(icon)<0 ){
        ...
        if (characterList.indexOf(icon)>=0){
            ...
            if (icon==5 || icon==6 | icon==10)frames[index].visible=false;
        }
    ...
    function swapSmb_  (icon,dest_,attr_){
        ...
        if (characterList.indexOf(icon)<0){
            if (game.cache.checkImageKey("smbBg_"+icon +"_"+numIcon_)){
                ...
                bg[dest] = game_.add.sprite(pos[dest].x + ReelConfig.icon.width / 2, poss, "smbBg_" + numIcon_);
            }
        ...
        if (attr == null ) {
            ...
            if (ic!=10) {
                ...
                img[dest] = game_.add.sprite(pos[dest].x + ReelConfig.icon.width / 2, pos[dest].y + height_ / 2, "icon" + ic+ "_"+numIcon_);
            }
        ...
        if (numIcon_>=5 && characterList.indexOf(icon)<0){
            var ratio=size_[numIcon_]/img[dest].height+((icon==10)?(gameParams.gameOriginalID=="7019")?.07:.3:.1);
            if (gameParams.gameOriginalID=="7025"){
                if (icon ==5 ||icon ==6 ||icon ==7 ){
                    ratio=size_[numIcon_]/img[dest].height-.05;
                }
            }
        ...
        if (characterList.indexOf(icon)>=0 ){
            ...
            iconAdjustment_(img[dest],icon,frames[dest]);
             if (icon==5 || icon == 6 ||icon==10)frames[dest].visible=false;
        }
    ...
    function newSmb_  (icon,dest_,attr_){
        ...
        if (attr == null || attr.reels==undefined || reel <0) {
            if (icon!=10) {
                ...
                img[dest] = game_.add.sprite(pos[dest].x + ReelConfig.icon.width / 2, pos[dest].y + height_ / 2, "icon" + icon+"_"+numIcon_);
            }
            img[dest].smbNum=icon;
            if (game.cache.checkImageKey("bicon" + icon)){
                imgBlurr[dest] = game_.add.sprite(pos[dest].x + ReelConfig.icon.width / 2, pos[dest].y + height_ / 2, "bicon" + icon);
