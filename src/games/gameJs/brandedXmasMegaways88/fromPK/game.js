function Game(gameRef) {
    ...
    function decreaseVolumeBG_(fadeT_O_){
        ...
        if (lastSound_>0) {
            for (var a = 1; a <= lastSound_; a++) {
                soundManager_.getBGSound("bgSlot" + a).fadeOutBgSoundExternal(500, (freeSpinsManager_.getIsInFs()?.3:.5));
            }
        }
        if (fadeT_O_!=null){
            fadeBg_=false;
    ...
    function increaseVolumeBG_(force){
        ...
        if (fadeBg_ || force) {
            ...
            if (lastSound_>0) {
                for (var a = 1; a <= lastSound_; a++) {
                    soundManager_.getBGSound("bgSlot" + a).fadeInBgSoundExternalJustVol(1000, audioLevel[a]);
                }
            }
        }
    }
...
return;
    ...
    function doBuyFeature_() {
        ...
        if ( spinning_==false && isTumbling_==false && (window.SPIN_STATE==undefined || window.SPIN_STATE.spinning!="true") ) {//todo ask Joe about mobile
