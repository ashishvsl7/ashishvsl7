/**
 * KiS Framework, Created by Fabry on 2/04/2020.
 * handle games with different wild reels (exmaple barbarian
 */
function MultipleWildsReel(gameRef, gameClass) {
    var game_ = gameRef;
            ...
            if (wildReelSmb_[i].pos[0] != undefined) {
                for (var a in wildReelSmb_[i].pos) {
                    if ( wildReelSmb_[i].anim[a] == undefined && wildReelSmb_[i].pos[a] != undefined) {
                        ...
                        }else if (game_.cache.checkImageKey("wildReel_" +spinManager_.getSpinResp().spin.reels[wildReelSmb_[i].simbolReference[0].reel].smb[0].smb+"_0") == true){
                            wildReelSmb_[i].anim[a] = game_.add.sprite(wildReelSmb_[i].pos[a].x + ReelConfig.icon.width / 2, wildReelSmb_[i].pos[a].y + ( ReelConfig.icon.height + ReelConfig.icon.height / 2), "wildReel_" +spinManager_.getSpinResp().spin.reels[wildReelSmb_[i].simbolReference[0].reel].smb[0].smb+"_0");
    ...
    function nextPart_(par){
        ...
        wildReelSmb_[i].anim[a]= game_.add.sprite(wildReelSmb_[i].anim[a].x , wildReelSmb_[i].anim[a].y, "wildReel_" + spinManager_.getSpinResp().spin.reels[wildReelSmb_[i].simbolReference[0].reel].smb[0].smb+"_1");
        wildReelSmb_[i].anim[a].animations.add("anim", animFrameSeq_);
        if (ReelConfig.wilTextAnimOnly!=true || spinManager_.getSpinResp().spin.reels[wildReelSmb_[i].simbolReference[0].reel].smb[0].smb>10) {
            wildReelSmb_[i].anim[a].animations.play("anim", (gameClass_.getCompulsivePlayer()) ? 48 : 24, false);
        }else{
            wildReelSmb_[i].anim[a].animations.play("anim", (gameClass_.getCompulsivePlayer()) ? 48 : 24, false,false,placeImages_,[i,a]);
        }
        ...
        if (communicationManager_.getResumeStatus().status!=null && communicationManager_.getResumeStatus().status.reels!=null) {
            ...
            for (var i = 0; i < resumeJson.length; i++) {
                if (wildManager_.getTriggerIcon().indexOf(resumeJson[i].smb[0].smb)>=0) {
                    ...
                    wildReelSmb_[i].anim[0] = game_.add.sprite(reels[i].getPos(a).x + ReelConfig.icon.width / 2, reels[i].getPos(a).y + ( ReelConfig.icon.height + ReelConfig.icon.height / 2), "wildReel_" + resumeJson[i].smb[0].smb+"_1");
