/**
 * KiS Framework, Created by Fabry on 13/01/2017.
 */

function LineCherryBlast(gameRef, gameClass) {
    ...
    function amimIconIdle_(winSmb_) {
        ...
        for (var b = 0; b < winSmb_.reel.length; b++) {
            ...
            if (winSmb_.anim[b].special!=undefined && winSmb_.anim[b].special=="mult"){
                var smbM = game_.add.sprite(winSmb_.pos[b].x + ReelConfig.icon.width / 2, winSmb_.pos[b].y + ReelConfig.icon.height / 2, "lights-mult");
                smbM.anchor.set(.5, .5);
        ...
        for (var a in smb.smbNum) {
            if (wildManager_.getWildReel(smb.reel[a])==false) {
                ...
                if (game.cache.checkImageKey("anim" + smb.smbNum[a]) == true) {
                    ...
                    if (smb.simbolReference[a].special!=undefined && smb.simbolReference[a].special=="mult"){
    ...
    var me = {
        ...
        getTotalDuration:function(){
            ...
            if (sAmt==0)sAmt=Number((winManager_.getScatterWinAmt()[0]!=undefined && winManager_.getScatterWinAmt()[0][10]!=null)?winManager_.getScatterWinAmt()[0][10].amt:0);
            if(sAmt>0){
