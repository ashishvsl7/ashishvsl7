 ...
 */
function SBBTumblingSpin(gameRef, gameClass) {
    var game_ = gameRef;
    ...
    function createReels_() {
        ...
        for (var i = 0; i < ReelConfig.numReels; i++) {
            ...
            reel_[i] = new SBBTumblingReel(game_, gameClass, i,i,null,numIcon_[i]);
            animationMap_[i]=[];
            c[i]=[];
    ...
    function createReelsAfterSpin_(reels){
        ...
        for (var i = 0; i < resumeJson.length; i++) {
            for (var a=0; a < resumeJson[i].smb.length; a++){
                ...
                if (gameParams.gameOriginalID==7025 && (resumeJson[i].smb[a].smb==11 || resumeJson[i].smb[a].smb==12 )&& resumeJson[i].smb[a].mult>0  ){
    ...
    function tumble_(reel,obj,reels) {
        ...
        for (var b = getSmallest_(obj.index); b<= numIcon_[reel]+obj.index.length;b++) {
            ...
            tumbleStarted[reel] = 1;
                ...
                if (characterList.indexOf(reel_[reel].getArr()[b].smbNum)>=0){
                    TweenMax.to( reel_[reel].getFrameMW(b) ,time,{delay:.5,y:c[reel][freePosIndex+1],ease:Bounce.easeOut});
                    if  (game.cache.checkImageKey("smbBg_"+reel_[reel].getArr()[b].smbNum +"_"+numIcon_[reel])      ){
                        TweenMax.to( reel_[reel].getTileMW(b),time,{delay:.5,y:c[reel][freePosIndex+1],ease:Bounce.easeOut}); //branded stones BG
                    }
                }else{
