        ...
        for (var a = 0; a < linesToDraw_; a++) {
            ...
            for (var w in winSmb.pos) {
                ...
                if ((w == 0 && startFromSinglePoint_ == true) || (w == winSmb.pos.length - 1 && endInSinglePoint_ == true)) {
                    pp = new Phaser.Point(0, 0);
                } else {
                    pp = new Phaser.Point(Math.random() * linesX_offset_ - Math.random() * linesX_offset_, Math.random() * linesY_offset_ - Math.random() * linesY_offset_);
                }
                if (w == 0) {   //first symbol
                    ll = new Phaser.Point(winSmb.pos[w].x + pp.x, winSmb.pos[w].y + smbH_ / 2 + pp.y);
                    randomIconPoints_[line][a].push(ll);
                }
                ll = new Phaser.Point(winSmb.pos[w].x + smbW_ / 2 + pp.x, winSmb.pos[w].y + smbH_ / 2 + pp.y);
            ...
            for ( var rest =Number(w)+1; rest<5 ;rest++){
                ...
                pp = new Phaser.Point(Math.random() * linesX_offset_ - Math.random() * linesX_offset_, Math.random() * linesY_offset_ - Math.random() * linesY_offset_);
                ll = new Phaser.Point(     smb.x + smbW_ / 2 + pp.x, smb.y + smbH_ / 2 + pp.y);
    ...
    function drawLine_(params) {
        ...
        for (a = 0; a < linesToDraw_; a++) {
            ...
            if (bezierIconPoints_[a].length > 0) {
                ...
                oldRandomIconPoints_[a] = new Phaser.Point(bezierIconPoints_[a][0].x, bezierIconPoints_[a][0].y);
    ...
    function updateTween_(line, a, i, pointsL) {
        if (container_[a] != null) {
            ...
            if (!isMobile) {
                ...
                container_[a].lineStyle(myLineTick_[a],  lineConfig.line.color, myAlphaLine_[a]);
                oldRandomIconPoints_[a] = new Phaser.Point(bezierIconPoints_[a][Math.ceil(objTwLine_[a].amt)].x, bezierIconPoints_[a][Math.ceil(objTwLine_[a].amt)].y);
