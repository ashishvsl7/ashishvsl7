function Game(gameRef) {
    ...
    stickyWildConf.wildType="ExpandingLStickyWild";
    ...
    function create_() {
        ...
        displayManager_.getGroup("seaLife").visible=true;
        displayManager_.getGroup("mermadeLevel").visible=true;
        if (Util.getRandomInt(1,100)>50){
            displayManager_.getGroup("mermadeLevel").mermade.children[0].visible=true;
        }else{
            displayManager_.getGroup("mermadeLevel").mermadeR.children[0].visible=true;
        }
    ...
    function initGame_(){
        ...
        if (communicationManager_.getResumeStatus().status.hand=="freespins") {
            ...
            idleCycleManager_.forceIdleCycle("s3"); //forces the logo to idle cycle
            idleCycleManager_.forceIdleCycle("s4"); //forces the logo to idle cycle
            idleCycleManager_.forceIdleCycle("s5"); //forces the logo to idle cycle
            idleCycleManager_.forceIdleCycle("s6"); //forces the logo to idle cycle
            idleCycleManager_.forceIdleCycle("s7"); //forces the logo to idle cycle
        }
        onGame_=true;
    ...
    function doSpin_() {
        ...
        if (buttonManager_.getButton(frameworkSettings.buttons.spinButton.id).isEnable()==true) {
            ...
            realDoSpin_([false]);
            var rand=Util.getRandomInt(1,100);
            if (rand>60)idleCycleManager_.forceIdleCycle("s1");
            if (rand>90)idleCycleManager_.forceIdleCycle("s2"); //forces the logo to idle cycle
            ...
            for (var i = 0; i < reels.length; i++) {
                for (var a = 0; a < 3; a++) {
                    ...
                    if (stickyWildManager_.getTriggerIcon().indexOf(spinManager_.getSpinReelResp().reel[i].smb[a].smb) >= 0 || ( spinManager_.getSpinReelResp().reel[i].smb[a].prevSmb != undefined && stickyWildManager_.getTriggerIcon().indexOf(spinManager_.getSpinReelResp().reel[i].smb[a].prevSmb >= 0))) {
                        if (spinManager_.getSpinReelResp().reel[i].smb[a].special != "done") {//attach a new one only when stage 1
    ...
    function fsLogoOut_(){
        ...
        displayManager_.getGroup("seaLife").visible=false;
        ...
        if (displayManager_.getGroup("idleObjects"))displayManager_.getGroup("idleObjects").visible=false;
        displayManager_.getGroup("mermadeLevel").visible=false;
        idleCycleManager_.stopIdleCycle();
        displayManager_.getGroup("mermadeLevel").mermade.children[0].visible=false;
        displayManager_.getGroup("mermadeLevel").mermadeR.children[0].visible=false;
    }
    ...
    function fsHideTotPanel_(){
        ...
        if (displayManager_.getGroup("idleObjects"))displayManager_.getGroup("idleObjects").visible=true;
        displayManager_.getGroup("mermadeLevel").visible=true;
        if (Util.getRandomInt(1,100)>50){
            displayManager_.getGroup("mermadeLevel").mermade.children[0].visible=true;
        }else{
