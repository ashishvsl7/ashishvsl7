 ...
 */
function PixiMegaWayRRTumblingWin(gameRef, gameClass) {
    ...
    function showLine_() {
            ...
            if (horizTransf_.length>0){
                for (var a=0 ;a<= horizTransf_.length;a++) {
                    if (horizTransf_[a]!=undefined) {
                        ...
                        winEventManager_.addEvent(gameClass_.animAnimalFrame, 1500, null, [horizTransf_[a]]);
        ...
        if (hasBeenCapped_ == 0) {
            ...
            if (tumbling_) winEventManager_.addEvent(restoreIcons, 800);
    ...
    function restoreIcon_(par){
        ...
        if (hasBeenCapped_==false) {
            ...
            smb.anim[a] = spriteManager_.addSpine(smb.simbolReference[a].x, smb.simbolReference[a].y, "tumble",null,null,1);
            displayManager_.getGroup("scatter").add_Child(smb.anim[a]);
            smb.anim[a].smbNum = smb.smbNum[a];
            smb.anim[a].iconNum = smb.icon[a];
            smb.anim[a].play("tumble_0"+spinManager_.getReels()[smb.reel[a]].getIconSize(0), false,removeTile_,par);
        }else{
    ...
    function delayedAnimation_(smb,a){
        ...
        smb.anim[a] = spriteManager_.addSpine(smb.simbolReference[a].x, smb.simbolReference[a].y, "anim" + ic,null,null,1);
...
//todo p3 multiplier
        ...
        smb.anim[a].play("win_0"+ spinManager_.getReels()[smb.reel[a]].getIconSize(0),false, restoreIcon_, [smb, a]);
    ...
    function changeCentralWinStage_(s,greetings){
        ...
        if (s==1 && stopped_==0) {
            centralWinMsgGroups_[s].play("win_tier_in",showFirstAnimCentral_);
            playWinningSounds_(tumblingWin_, true);
        }else if (s>1 && stopped_==0) {
            centralWinMsgGroups_[1].play("win_tier_0"+(s),false);
        }
        soundManager_.stopSound(winAnim.accuSnd);
        centralWinMsgGroups_[1].visible=true;
        centralWinMsgGroups_[1].alpha=1;
        centralWinMsgGroups_[1].play("win_tier_in",false);
    ...
    function showFirstAnimCentral_(){
        centralWinMsgGroups_[1].play("win_tier_01",false);
    }
