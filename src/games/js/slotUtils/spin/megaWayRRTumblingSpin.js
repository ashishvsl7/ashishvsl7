 ...
 */
function MegaWayRRTumblingSpin(gameRef, gameClass) {
    var game_ = gameRef;
    ...
    function createReels_() {
        ...
        for (var i = 0; i < ReelConfig.numReels; i++) {
            ...
            reel_[i] = new MegaWaysRRTumblingReel(game_, gameClass, i,i,null,numIcon_[i]);
            animationMap_[i]=[];
            c[i]=[];
            cBeforeSplit[i] = [];
    ...
    var me = {
        ...
        getFrameH:function (reel,index){
            return reel_[reel].getFrameH(index);
        },
