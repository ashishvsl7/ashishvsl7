 ...
 */
function PixiMegaWayTumblingSpin(gameRef, gameClass) {
    var game_ = gameRef;
    ...
    function createReels_() {
        ...
        for (var i = 0; i < ReelConfig.numReels; i++) {
            ...
            reel_[i] = new PixiMegaWaysTumblingReel(game_, gameClass, i,i,null,numIcon_[i]);
            animationMap_[i]=[];
            c[i]=[];
        ...
        if (!hasToWait_) {
            ...
            for (var i = 0; i < resumeJson.length; i++) {
                for (var a=0; a < resumeJson[i].smb.length; a++){
                    ...
                    if (gameParams.gameOriginalID==7034 && (resumeJson[i].smb[a].smb==11 || resumeJson[i].smb[a].smb==12 )&& resumeJson[i].smb[a].mult>0  ){
