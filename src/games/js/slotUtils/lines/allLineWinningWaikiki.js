 ...
 */
function AllLineWinningsWaikiki(gameRef, gameClass) {
    ...
    var winAnim = {
        "winClasses_":    [1,   10,  30,  80, 120, 240, 600, 1500, 3000],
    ...
    function showLine_() {
        ...
        soundManager_.playSound("winBg"  +((freeSpinsManager_.getIsInFs()==false)?"":"FS") +  "_"+ winAnim.winSound_[index]);   //total bg sound
    ...
    function showCentralWin_(params){
        ...
        if (winAnim.bigWinText[index]=="1" ){    // show coins animation
            ...
            console.log("low win " +me.getTotalDuration() )
    ...
    function amimIcon_(params) {
        ...
        for (var b = 0; b < winSmb_.reel.length; b++) {
            if (wildManager_.getWildReel(winSmb_.reel[b]) == false) {
                if (gameClass_.getStickyWildManager().isSymbolWilded(winSmb_.reel[b],winSmb_.icon[b])!=true) {
                    hlSmb([spinManager_.getAnimationMap(winSmb_.reel[b], winSmb_.icon[b])]);
                }else{
    ...
    function delayedAnimation_(smb,a){
        ...
        console.log("not wild " +smb.reel[a]+  " "  +smb.icon[a] )
        if  ( gameClass_.isSymbolWilded(smb.reel[a],smb.icon[a])==true)return;
        ...
        if (spinManager_.getAnimationMap(smb.reel[a],smb.icon[a]).animations==null || ReelConfig.keepShowingAnimation==true) //avoid to add the same animation more then once
        {
            ...
            if (game.cache.checkImageKey("anim" + smb.smbNum[a])){
                smb.anim[a] = game_.add.sprite(smb.pos[a].x + ReelConfig.icon.width / 2, smb.pos[a].y + ReelConfig.icon.height / 2, "anim" + gameClass_.getFsIcon(smb.smbNum[a])+ smb.smbNum[a]);
                ...
                smb.anim[a] = game_.add.sprite(smb.pos[a].x + ReelConfig.icon.width / 2, smb.pos[a].y + ReelConfig.icon.height / 2, "icon"+ gameClass_.getFsIcon(smb.smbNum[a]) + smb.smbNum[a]);
                smb.anim[a].anchor.set(.5, .5);
                smb.anim[a].scale.y = 1;
                smb.anim[a].smbNum = smb.smbNum[a];
                smb.simbolReference[a].visible = false;
                ...
                //animGroup_.add(smb.anim[a]);
                spinManager_.setAnimationMap(smb.reel[a], smb.icon[a], smb.anim[a]);
                if (smb.reel[a]<2 && ReelConfig.simbolsReflection!=undefined && ReelConfig.simbolsReflection.indexOf(smb.smbNum[a])>=0){
                    smb.anim[a].scale.x=-1;
                    smb.anim[a].inversion=true;
                }else{
                    smb.anim[a].scale.x=1;
                    smb.anim[a].inversion=false;
                }
                scaleBack_(smb.anim[a]);
            }
        ...
        if (smb.anim[a].scale.x <=1) {
            if ((add==true || lineConfig.line.multipleAnimation[smb.smbNum[a]]==true)&& game.cache.checkImageKey("anim" + smb.smbNum[a])) {
    ...
    function scaleBack_(anim){
        ...
        console.log("inversion " + anim.inversion);
        TweenMax.to(anim.scale,.3,{delay:.2,x:(anim.inversion)?-1.10:1.10,y:1.10,onComplete:riScaleBack_,onCompleteParams:[anim]})
    }
    ...
    function scaleBack1_(anim){
        ...
        TweenMax.to(anim.scale,.3,{delay:.2,x:(anim.inversion)?-1.10:1.10,y:1.10,onComplete:riScaleBack1_,onCompleteParams:[anim]})
    }

    function riScaleBack_(anim){
        TweenMax.to(anim.scale,.3,{delay:.2,x:(anim.inversion)?-1:1,y:1,onComplete:scaleBack1_,onCompleteParams:[anim]})
    }

    function riScaleBack1_(anim){
        TweenMax.to(anim.scale,.3,{delay:.2,x:(anim.inversion)?-1:1,y:1,onCompleteParams:[anim]})
    }

    function restoreIcon_(par){
        var smb=par[0];
        if (smb==null || smb == undefined)return;
        if (smb.scale>1)TweenMax.to(smb.scale, .1, {x: (smb.inversion)?-1:1, y: 1});
    }
    ...
    function showBonusCentralWinGreetings_(amt){
        ...
        if (winAnim.bigWinText[index]=="1"){    // show coins animation
            ...
            soundManager_.playSound("winBgS" +((freeSpinsManager_.getIsInFs()==false)?"":"FS")+   "_"+winAnim.winSound_[index]);
    ...
    function startNewSmallObject_(){
        ...
        TweenMax.to(ob, .5 + Util.getRandomInt(20, 50) / 100, {
            alpha: 1,
            rotation:Util.getRandomInt(0, 5),
    ...
    function startNewObject_(){
        ...
        TweenMax.to(ob, .6 + Util.getRandomInt(20, 50) / 100, {
            alpha: 1,
            rotation:Util.getRandomInt(0, 5),
