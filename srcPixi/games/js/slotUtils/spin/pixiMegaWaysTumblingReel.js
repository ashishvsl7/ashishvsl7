function  PixiMegaWaysTumblingReel(gameRef,gameClass,_num,num2,swap,len){
    var game_=gameRef;
    ...
    function setIcon_(index,icon){
        ...
        imgBlurr[index] =  spriteManager_.addSprite( pos[index].x + ReelConfig.icon.width / 2,  poss,["bicon"+ icon ,iconSize_[index]-2]);
    ...
    function swapSmb_  (json,dest_,attr_){
        ...
        imgBlurr[dest] =  spriteManager_.addSprite(pos[dest].x + ReelConfig.icon.width / 2, poss,["bicon"+ icon ,iconSize_[dest]-2 ]);
    ...
    function replaceIcon2_(dest_,icon,attr_){
        ...
        if (attr_==undefined){
            ...
            if(bg[dest]!=undefined)group.removeChild(bg[dest]);
            img[dest] = spriteManager_.addSpine(pos[dest].x + ReelConfig.icon.width / 2, poss,"anim"+ icon ,null,null,.25);
            img[dest].smbNum=icon;
            img[dest].reel=num;
            group.add_Child(img[dest]);
            img[dest].play("win_7_0" + (iconSize_[dest]).toString(), false, attr_.callBack, attr_.param);
            iconN[dest]=icon;
        }
