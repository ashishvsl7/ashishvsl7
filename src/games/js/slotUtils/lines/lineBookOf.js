 ...
 * handle line and scatters fakin glines for book of engine
 */
function LineBookOf(gameRef, gameClass) {
    ...
    function initClass_() {
        ...
        for (var a in displayManager.groups.centralLineWin.buttons) {
            buttonE = new Phaser.Button(game_, displayManager.groups.centralLineWin.buttons[a].x, displayManager.groups.centralLineWin.buttons[a].y, displayManager.groups.centralLineWin.buttons[a].name, me[displayManager.groups.centralLineWin.buttons[a].evt], this, 1, 2, 0, 0);
            buttonE.alpha=0;
            buttonE.anchor.set(0.5, 0.5);
            coinWinGroup_.add(buttonE);
        }

    }

    var winAnim = {
        "winClasses_":    [0,   10,  20,  50, 100, 200, 250, 500, 2000],
        "winClassesBI_":    [0,   1,  1,  1, 1, 1, 2, 5, 10],
        "winSound_":    [0,   2,  3,  4, 5, 5, 6, 7, 8],
        "winTxtDuration": [.5,  1.5, 1.5, 1.5,1.7, 1.8, 3, 3, 3],
        "centralWinDur": [0.5, .7, 1, 2, 2, 2, 3, 3, 3],
        "scaleFactor": [1.4, 1.45, 1.5, 1.55, 1.6,1.65, 1.7, 1.8, 2],
    ...
    function showBonusWin_(amt,amtCash){
        ...
        reset_=false;
        displayManager_.getGroup("centralLineWin").visible=true;
        ...
        if (ReelConfig.winType=="BounceWin")
            winAmtClass_= new BounceWin(game_,gameClass_, winAnimB,"lineWinFinal");
        if (ReelConfig.winType=="SimpleWin")
            winAmtClass_= new SimpleWin(game_,gameClass_, winAnimB,"lineWinFinal");
    ...
    function showLine_() {
        ...
        if (wins.length>0) winEventManager_.addEvent(changeAllHL_, 200, [.35]);
        allLineDraw_=[];
        for (var a in wins) {
            if (wins[a] != undefined) {
                if (wins[a].winAmount > 0) {
                    ...
                    if(loginVars.getRegParams().jurisdiction.toUpperCase()=="IT") { //all lines must show for IT
                        if (checkBrowser_()) {
        ...
        index=findWinRange_(winManager_.getLineWinAmt());
        if(Number(winManager_.getLineWinAmt())>0)winEventManager_.addEvent(showCentralWinLine_,10,[index]);
        ...
        if (freeSpinsManager_.getIsInFs()==true){//book of expanding symbols and paylines
            //check if scatter win
            if (winManager_.getScatterWinAmt() ){
                if ( winManager_.getScatterWinAmt()[0].id!=9 || winManager_.getScatterWinAmt().length>1) {
                    winEventManager_.addEvent(fakeEvent,winManager_.getLineWinAmt()>0?2000:500);
                    //grey all the symbols
                    winEventManager_.addEvent(changeSaturation_, (winAnim.winTxtDuration[index] * 1000) + 500, [0.35, winManager_.getScatterWinAmt()[0].id]);
                    //expands simbols
                    winEventManager_.addEvent(expandsSymbols_, 500);
                    //run fake paylines
                    winEventManager_.addEvent(allLinesScatter_, 1200);
                    //show total win amount
                }
            }
            index=findWinRange_(winManager_.getWinAmt());
            winEventManager_.addEvent(showCentralWinScatter_,200,[index]);
        }
    ...
    function showCentralWinScatter_(params){
        var index=findWinRange_(winManager_.getWinAmt());
        winAmtClass_.showWin(winManager_.getWinAmt(),null,null,null, winManager_.getLineWinAmt(), null,winAnim.scaleFactor[index],winAnim.winTxtDuration[index]);
        gameClass_.decreaseVolumeBG( winAnim.centralWinDur[index]);
        soundManager_.playSound("winBg_"+ winAnim.winSound_[index]);   //total bg sound
    ...
    function showCentralWinLine_(params){
    ...
    function hlSmb(par) {
        ...
        a.alpha = 1;
        if (a.scatterIcon!=undefined){
            a.scatterIcon.alpha=1;
        }
    }
    ...
    function updateBalance_(){
        ...
        if (freeSpinsManager_.getIsInFs()==false || freeSpinsManager_.getFsNumber()==-999) {
            TweenMax.to(coinWinTxt_, .4, {y: 500, alpha: 0.1, onComplete: startUpdate, ease: Sine.easeInOut});
        }else{
    ...
    function updateBonusBalance_(amt,msg){
        reset_=true;
        TweenMax.to(coinWinTxt_, .4, {y: 500, alpha: 0.1, onComplete: startBonusUpdate,onCompleteParams:[amt,msg], ease: Sine.easeInOut});
        soundManager_.playSound("line");
        resetCentralWin_();
    }
    ...
    function expandsSymbols_(params) {
        gameClass_.playTriggeredSmb(null);
    }

    function allLinesScatter_(params) {
        var smbW_ = ReelConfig.icon.width;
        var smbH_ = ReelConfig.icon.height;
        var pp;
        var real;
        ...
        for (var line in lineConfig.line.lines) {
            coinsIconPoints_[line] = [];
            for (var rest = 0; rest < 5; rest++) {
                var smb = spinManager_.getReels()[rest].getPos((Number(lineConfig.line.lines[line].config[rest])));
                if (rest==0) {
                    real = new Phaser.Point(smb.x + pp.x, smb.y + smbH_ / 2 + pp.y);
                    coinsIconPoints_[line].push(real);
                }else if (rest==4){
                    real = new Phaser.Point(smb.x + smbW_, smb.y + + smbH_ / 2);
                    coinsIconPoints_[line].push(real);
                }else{
                    real = new Phaser.Point(smb.x + smbW_ / 2, smb.y + smbH_ / 2);
                }
            ...
            coinsIconPoints_[line].push(real);
            if (checkBrowser_()) {
                allLineDraw_[line] = new RopeDraw(game_, gameClass);
            } else {
                allLineDraw_[line] = new CCurveDraw(game_, gameClass);
            }
            allLineDraw_[line].showStatic(0, line, 0, coinsIconPoints_[line], 2000);

        }

    }

    function changeSaturation_(params) {
        var alpha = params[0];
        var specialSmb = params[1];
        var reels = spinManager_.getReels()

        resetCentralWin_();
        clearAll_();
        changeAllHL_([1]);//restore alpha first
        for (var a in reels) {
            for (var s = 0; s < ReelConfig.numIcons; s++) {
                //reels[a].getSymbol(s).tint = "#CCCCCC";
                var smb = reels[a].getSymbol(s);
                if (smb.smbNum < gameClass_.getHlMaximumIcon() && smb.smbNum != specialSmb) smb.alpha = alpha;
                if ( smb.smbNum == specialSmb){
                    var smbn = new Phaser.Sprite(game_, smb.x, smb.y, "anim" + smb.smbNum );
                    smbn.anchor.set(.5, .5);
                    smbn.animations.add("anim");
                    smbn.scale.x = 1;
                    smbn.scale.y = 1;
                    displayManager_.getGroup("scatter").add(smbn);
                    smbn.animations.play("anim", 24, false, true);
        ...
        for (var a in map) {
            if (map[a]!=null) {
                for (var b in map[a] ) {
                    if (map[a][b].smbNum!=undefined && map[a][b].smbNum<gameClass_.getHlMaximumIcon()&& map[a][b].smbNum !=specialSmb) {
                        map[a][b].alpha = alpha;
                    }else{
                        if(map[a][b].smbNum ==specialSmb){
                            map[a][b].animations.play("anim", 24, false, false);
                        }
                    }
                }
            }
        ...
        for (var a in reels) {
            for (var s = 0; s < ReelConfig.numIcons ; s++) {
                ...
                if (smb.smbNum<gameClass_.getHlMaximumIcon())
                    smb.alpha = alpha;
                    if (smb.scatterIcon!=undefined){
                        smb.scatterIcon.alpha=alpha;
                    }
            ...
            if (map[a]!=null) {
                for (var b in map[a] ) {
                    if (map[a][b].smbNum!=undefined && map[a][b].smbNum<gameClass_.getHlMaximumIcon()) {
                        map[a][b].alpha = alpha;
                        if (map[a][b].scatterIcon!=undefined){
                            map[a][b].scatterIcon.alpha=alpha;
                        }
                    }
                }
            }
    ...
    function delayedAnimation_(smb,a){
        ...
        if (smb.anim[a].scale.x <=1) {
            if ( add==true || lineConfig.line.multipleAnimation[smb.smbNum[a]]==true)
                smb.anim[a].animations.play("anim", 24, false, false, restoreIcon_, [smb.simbolReference[a],spinManager_.getAnimationMap(smb.reel[a], smb.icon[a])]);//todo parametric loop
            else
                setTimeout(restoreIcon_,500,[smb.simbolReference[a],spinManager_.getAnimationMap(smb.reel[a], smb.icon[a])]);
        }
