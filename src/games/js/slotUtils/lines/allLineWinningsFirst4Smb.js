 ...
 */
function AllLineWinningsFirst4Smb(gameRef, gameClass) {
    ...
    var winAnim = {
        "winClasses_":    [1,   10,  30,  80, 120, 240, 1000, 2500, 5000],
        "winSound_":    [1,   2,  3,  3, 4, 5, 6, 7, 8],
        "winTxtDuration": [1,  1.7, 2.2, 2.5,   2.7, 3.5, 5,   6, 6   ],
        "centralWinDur": [0, 0, 0, 0, 0, 0, 6, 6, 7],
    ...
    function amimIconIdle_(winSmb_) {
        ...
        for (var b = 0; b < winSmb_.reel.length; b++) {
            ...
            if (gameClass_.getStickyWildManager().isSymbolWilded(winSmb_.reel[b],smbIndex)!=true) {
            ...
            if (winSmb_.anim[b].special!=undefined && winSmb_.anim[b].special=="mult"){
                var smbM = game_.add.sprite(winSmb_.pos[b].x + ReelConfig.icon.width / 2,( winSmb_.pos[b].y + ReelConfig.icon.height / 2), "multiplier");
                smbM.anchor.set(0, 1);
                smbM.scale.x = .5;
                smbM.scale.y = .5;
                smbM.reel=winSmb_.reel[b];
                spinManager_.getReelGroup(winSmb_.reel[b]).add(smbM);
                TweenMax.to(smbM,1,{alpha:0, delay:1,onComplete:endMult_,onCompleteParams:[smbM]});
    ...
    function delayedAnimation_(smb,a){
        ...
        if ( gameClass_.isReelWilded(smb.reel[a])==true)return;
        ...
        if (spinManager_.getAnimationMap(smb.reel[a],smb.icon[a]).animations==null || ReelConfig.keepShowingAnimation==true) //avoid to add the same animation more then once
        {
            ...
            if (spinManager_.getReels()[smb.reel[a]].getSymbol(smb.icon[a]).smbNum==14){
                smb.anim[a] = game_.add.sprite(smb.pos[a].x + ReelConfig.icon.width / 2, smb.pos[a].y + ReelConfig.icon.height / 2, "anim" + 14);
            }else{
    ...
    function endMult_(smb) {
        var reel=smb.reel;

        spinManager_.getReelGroup(reel).remove(smb);
        smb=null;
    }
