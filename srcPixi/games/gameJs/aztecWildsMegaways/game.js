function Game(gameRef) {
    ...
    function setPortrait_(port){
        ...
        if (port) {
            ...
            adaptPortraitAsset_(displayManager_.getGroup("logo"), (scaleW_/2 ) -(360*xRatio_),deviceYoffset,xRatio_*1.2);
            adaptPortraitAsset_(displayManager_.getGroup("bg"), (scaleW_/2 ) -(960*xRatio_)*.9,yPositionOffest+deviceYoffset -130*xRatio_,xRatio_*0.9);
            adaptPortraitAsset_(displayManager_.getGroup("bgFS"), (scaleW_/2 ) -(960*xRatio_)*.9,yPositionOffest+deviceYoffset -130*xRatio_,xRatio_*0.9);
            ...
            setPorytraitAssets_(displayManager_.getGroup("preview").children[5],true,400 ,1760);
            setPorytraitAssets_(displayManager_.getGroup("preview").children[6],null,null ,730);
        }else{
            ...
            setPorytraitAssets_(displayManager_.getGroup("preview").children[5],null,50 ,1005);
    ...
    function hideGame_(flag){
        turning_=flag;https://www.1x2networkhub.com/f1x2games/installs/loadGameStandard.jsp?platform=desktop&display=flash&gameID=7022&lang=en&sessionID=0&playMode=fun&siteID=13&jurisdiction=&realitycheck_uk_elapsed=0&realitycheck_uk_limit=0&realitycheck_uk_proceed=http://&realitycheck_uk_exit=http://&realitycheck_uk_history=http://&realitycheck_uk_autospin=http://&demo=ids&forceMobile=true&end=true&clientName=Machine_13&site=13
    ...
    function attachAnim_(){
        ...
        var obj = spriteManager_.addSpine(200,500, "anim1",null,null,.25);
        displayManager_.getGroup("scatter").add_Child(obj);
        obj.play("win_7_02",false);
    ...
    function fadeInFs_() {
        ...
        displayManager_.getText("win").setText("");
        if (autoPlayManager_.getStopOnFeature()==true){
            buttonManager_.applyState("NHFS");
        }
    }
...
//    return
        ...
        if (sucessiveWin_!= anchorPos_ ) {
            if (sucessiveWin_>0){
                if (sucessiveWin_<=4) {
                    pot_.play( sucessiveWin_+"up",false);
                    soundManager_.playSound("potFill_0"+sucessiveWin_);
                }
                if (sucessiveWin_==1)displayManager_.getText("win").setText("");
            }else if (sucessiveWin_==0){
                if (anchorPos_>0){
                    pot_.play(Math.min(anchorPos_,4)+"down",false);
                }else if  (anchorPos_<0){
                    pot_.play( Math.max(anchorPos_,-4)+"down",false);
                }else{
                    pot_.play(sucessiveWin_,false);
                }
                soundManager_.playSound("potReset");
                displayManager_.getText("win").setText("");
            }else if (sucessiveWin_> -5){
                pot_.play(sucessiveWin_+"up",false);
                soundManager_.playSound("potFill_0"+Math.abs(sucessiveWin_));
                clearTimeout(quiteSpinMusic);
            }
            if (freeSpinsManager_.getIsInFs()==true){
                if (sucessiveWin_>=4){
                    displayManager_.getText("win").setText("+"+sucessiveWin_-2+"FS");
                }else if  (sucessiveWin_<=-4){
                    displayManager_.getText("win").setText("+" +Math.abs(sucessiveWin_)-2+"FS")
                }
            }
        }
    ...
    function spinAnimEnd_() {
        console.log("TESTING game.js - > 1023 : spinAnimEnd_ ");
    ...
    function evaluateWinnings_(){
        ...
        slotState_=spinManager_.getSpinResp().state;
        ...
        if (spinManager_.getSpinResp()!=undefined){
            if (spinManager_.getSpinResp().state.wonAdditionalSpins>0) {
                displayManager_.getText("win").setText(Translate.do("+"+ spinManager_.getSpinResp().state.wonAdditionalSpins + "FS"));
    ...
    function spinAnimEndAfterTumbling_(){
        console.log(" sEAT "+ isTumbling_ + " "+lineManager_.getInterruptedWinAnim())
        if (isTumbling_==false && lineManager_.getInterruptedWinAnim()==false ) {
            ...
            if (won>0 || hasScatter_){
                ...
                freeSpinsManager_.endAnimHandle(eventManager_);
                if(fsWon){
                    displayManager_.getText("win").setText(Translate.do("+"+ spinManager_.getSpinResp().state.freespinsRemaining + "FS"));
    ...
    function playTriggeredSmb_(){
        ...
        for (var a in triggerSymbol_){
            if ( triggerSymbol_[a].played==undefined) {
                if (triggerSymbol_[a].tnt==true) {
                    ...
                    var s = spinManager_.getReels()[triggerSymbol_[a].reel].replaceIcon2(triggerSymbol_[a].smb,obj.icon[0] , obj);
                    add = 2;
                    triggerSymbol_[a].smb=s;
                    triggerSymbol_[a].played = true;
                    setTimeout( placeMult_, 500,triggerSymbol_[a]);
                }
        ...
        if (add==1){
            ...
            soundManager_.playSound("smbWin_12");
        }
        placeMashroomWilds_ =false;
        placemWildsMults_=false;
    }
    ...
    function placeMult_(obj) {
        //moving text according to symbol size
        var h= 195;
        if (obj.smb.size==3){
            h= 50;
        }else if (obj.smb.size==4){
            h= 30;
        }else if (obj.smb.size==5){
            h= 20;
        }else if (obj.smb.size==6){
            h= 20;
        }else if (obj.smb.size==7){
            h= 15;
        }
        //using pot text font
        var t =spriteManager_.addText(0 , h,  ("x"+obj.mult),.5,.5, {
            fontFamily: displayManager.groups.pot.animAsset.pot.texts.win.font,
            fontSize: displayManager.groups.pot.animAsset.pot.texts.win.fontSize,
        });
        t.style=displayManager.groups.pot.animAsset.pot.texts.win.style
        t.scale.x=5;
        t.scale.y=5;
        obj.smb.addChild(t);
    }
    ...
    function addWilds_(a){
        ...
        obj.smbN=triggerSymbol_[a].smb;
        // obj.icon[0]=13;
        // spinManager_.getReels()[triggerSymbol_[a].reel].replaceIcon2(triggerSymbol_[a].smb, 13,obj);
    }
