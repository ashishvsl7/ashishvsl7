...
function AllLineWinningsFirst(gameRef, gameClass) {
    ...
    function calcSingleLinePoints_(line, winSmb) {
        ...
        for (var w in winSmb.pos) {
            ...
            pp = new PIXI.Point(lineConfig.line.x_origin, 0);
            if (w == 0) {   //first symbol
                real = new PIXI.Point(winSmb.pos[w].x + pp.x, winSmb.pos[w].y + smbH_ / 2 + pp.y);
                coinsIconPoints_[line].push(real);
            }
            real = new PIXI.Point(winSmb.pos[w].x + smbW_ / 2, winSmb.pos[w].y + smbH_ / 2);
            coinsIconPoints_[line].push(real);
        }
        ...
        for ( var rest =Number(w)+1; rest<5 ;rest++){
            ...
            real = new PIXI.Point(smb.x + smbW_ / 2, smb.y + smbH_ / 2);
            coinsIconPoints_[line].push(real);
        }
        coinsIconPoints_[line].push(real);

        //invert here when FS type fight for Avalon
        //coinsIconPoints_[line].reverse();
        ...
        for (var a in wins) {
            if (wins[a] != undefined) {
                if (wins[a].winAmount > 0) {
                    ...
                    if(loginVars.getRegParams().jurisdiction.toUpperCase()=="IT" && Number(gameParams.gameID > "7021") ) { //all lines must show for IT
    ...
    function delayedAnimation_(smb,a){
        ...
        if (spinManager_.getAnimationMap(smb.reel[a],smb.icon[a]).animations==null || ReelConfig.keepShowingAnimation==true) //avoid to add the same animation more then once
        {
            ...
            smb.anim[a] =spinManager_.getReels()[smb.reel[a]].swapSmb(smb.smbNum[a], smb.icon[a],"spine");
        ...
        if (smb.anim[a].scale.x <=1) {
            if ( add==true || lineConfig.line.multipleAnimation[smb.smbNum[a]]==true)
                smb.anim[a].state.setAnimation(0, 'win', false);
            else
