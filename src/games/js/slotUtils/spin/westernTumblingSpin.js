 ...
 */
function WesternTumblingSpin(gameRef, gameClass) {
    var game_ = gameRef;
    ...
    function createReelsAfterSpin_(){
        ...
        if (spinManager_.getSpinReelResp()!=undefined && spinManager_.getSpinReelResp().reel!=null){
            ...
            triggerSymbol_=[]
            for (var i = 0; i < resumeJson.length; i++) {
                for (var a=0; a < ReelConfig.numIcons; a++)resetSmb_(resumeJson,i,a);
            }
    ...
    function resetSmb_(resumeJson,i,a){
        ...
        if(resumeJson[i].smb[a]!=undefined) {
            if (resumeJson[i].smb[a].smb == 13 && gameParams.gameOriginalID=="7017") {
                ...
                obj = null;
                if (resumeJson[i].smb[a].special!="remove"){
                    var s=reel_[i].swapSmb((resumeJson[i].smb[a].prevsmb!=null)?resumeJson[i].smb[a].prevsmb:resumeJson[i].smb[a].smb, a, obj);
                    var newSmb = (resumeJson[i].smb[a].prevsmb != null) ? resumeJson[i].smb[a].prevsmb : resumeJson[i].smb[a].smb;
                    if (newSmb == 11 && resumeJson[i].smb[a].special == "tumble") { //check for special to animate1
                        s.reel = i;
                        s.smb = a;
                        s.trigger = true;
                        s.prevSmb=resumeJson[i].smb[a].prevsmb;
                        s.smbN = resumeJson[i].smb[a].smb;
                        s.tnt = true;
                        spinManager_.addTriggerIcon(s);
                        tnt = true;
                        gameClass_.addExplosionDelay();
                    }

                    if (resumeJson[i].smb[a].animate == true && resumeJson[i].smb[a].prevsmb != null && resumeJson[i].smb[a].prevsmb != 11) { //check for special to animate
                        s.reel = i;
                        s.smb = a;
                        s.tnt = false;
                        s.smbN = resumeJson[i].smb[a].smb;
        ...
        if (tnt){
            setTimeout(gameClass_.playTriggeredSmb,100);
        }
    ...
    function tumbleByReel_(reel,obj) {
        ...
        for (var b = getSmallest_(obj.index); b<= ReelConfig.numIcons+obj.index.length;b++) {
            ...
            if ( ind>=0) {   //tumbling icon
                ...
                if (dust==false  && gameParams.gameOriginalID!="7024"  ){
                    dust=true;
    ...
    function tumble_(reel,obj,json) {
        if (gotError_)return;
        var time=.35;
        freePos=[];
        ...
        var tnt=false;
        var numNewIcons=0;
        var icons= json[reel].smb.length;

        if (json[reel].smb[0].special=="remove"){
            icons=icons-1;
        }
        if (json[reel].smb[5].special=="remove"){
            icons=icons-1;
        }
        ...
        for (var b = getBigger_(obj.index); b>= -1*(icons);b--) {
            ...
            if ( ind>=0  ) {   //tumbling icon
                ...
                if ((icons >4 &&  b<1) || (icons== 4  && b<1 )) { //outside screen, need to replace with right icon
                    //outside reels, put a new symbol first
                    var newSmb = ( obj.newSmbs.smb[freePosIndex].prevsmb!=undefined)? obj.newSmbs.smb[freePosIndex].prevsmb:obj.newSmbs.smb[freePosIndex].smb;

                    var s=reel_[reel].replaceIcon(c[reel][(b)],newSmb);
                    if (newSmb==11) setTimeout(delayedLandSound_,250);

                }else{
                    if ( reel_[reel].getWholeArrSmb(c[reel][(b)]).smbNum==14){
                        var newSmb = ( obj.newSmbs.smb[freePosIndex].prevsmb!=undefined)? obj.newSmbs.smb[freePosIndex].prevsmb:obj.newSmbs.smb[freePosIndex].smb;
                ...
                TweenMax.to(reel_[reel].getWholeArrSmb(c[reel][(b)]),time,{delay:.1,y:reel_[reel].getWholeArrSmb(c[reel][freePosIndex]).y,ease:Bounce.easeOut});
                setTimeout(changeIcon, 200,reel,reel_[reel].getWholeArrSmb(c[reel][freePosIndex]).y,obj.coinsWin);
                freePosIndex--;
    ...
    function changeIcon(reel,y,win){
        if (Util.getRandomInt(1,100)>50) {
            var winAmt=(win>50)?3:(win>20)?2:1;
            var expl = game_.add.sprite(ReelConfig.reel.deltaX_0 + (ReelConfig.reel.deltaX * reel) + ReelConfig.reel.deltaX / 2, y + 2 * ReelConfig.reel.deltaY, "tumble_"+winAmt);
            expl.anchor.set(.5, .5);
            expl.animations.add("anim");
            expl.scale.x = 1;
            expl.scale.y = 1;
            ...
            //var expl = game_.add.sprite(ReelConfig.reel.deltaX_0 + (ReelConfig.reel.deltaX * reel) + ReelConfig.reel.deltaX / 2, y + 2 * ReelConfig.reel.deltaY, "tumble-smoke");
        }
    }
