 ...
 */
function MegaWayTumblingWinnings(gameRef, gameClass) {
    ...
    var winAnim = {
        ...
        "scaleFactor": [2, 2.5, 2.6, 2.7, 3, 3.2, 3.4, 3.5, 4],
    ...
    var disableOk_=false;
    function doOk_() {
        if (hasBeenCapped_>0)return;
        if (disableOk_)return;
        if(spinManager_!=undefined && spinManager_.getFsResp()!=undefined &&spinManager_.getFsResp().won>0)return;
        disableOk_=true;
    ...
    function showLine_() {
        ...
        if (hasBeenCapped_==0) {
            ...
            if (tumbling_)winEventManager_.addEvent(tumble_,1200);
    ...
    function showCentralWin_(params){
        ...
        if (freeSpinsManager_.getIsInFs()==true) {
            coinWinTxt_.y=(getOrientation()=="portrait")?330: (framework.isTouch())?640:630;
        }
        ...
        if (isBrandedMw())soundManager_.playSound("lock");
        ...
        if (last){
            ...
            if (index >= 0 ) {
                ...
                if (index>4) {
                    ...
                    funtainEventManager_.triggerEvt();
                    if (isBrandedMw())gameClass_.decreaseVolumeBG(me.getTotalDuration()+2000);
                }
        ...
        if (spinManager_.getAnimationMap(smb.reel[a],smb.icon[a]).animations==null || ReelConfig.keepShowingAnimation==true) //avoid to add the same animation more then once
        {
            ...
            if (game.cache.checkImageKey("anim" + smb.smbNum[a])==true) {
                ...
                if (smb.smbNum[a]==7 ||smb.smbNum[a]==8 ||smb.smbNum[a]==9){
                    //console.log(spinManager_.getFrameMW(smb.reel[a],smb.icon[a]))
            ...
            if (spinManager_.getSpinReelResp().reel[smb.reel[a]].smb.length>=5 && ((smb.smbNum[a]!=7 && smb.smbNum[a]!=8 && smb.smbNum[a]!=9)|| gameParams.gameOriginalID=="7030")){
                ...
                if (gameParams.gameOriginalID=="7030"){
                    var numIcon_=spinManager_.getSpinReelResp().reel[smb.reel[a]].smb.length;
                    if (smb.smbNum[a] ==5 ||smb.smbNum[a] ==6 ||smb.smbNum[a] ==7 ){
                        if (numIcon_==5){
                            ratio=size_[numIcon_]/smb.anim[a].height-.13;
                        }else if (numIcon_==6){
                            ratio=size_[numIcon_]/smb.anim[a].height+0;
                        }else if (numIcon_==7){
                            ratio=size_[numIcon_]/smb.anim[a].height+0;
                        }
                    }else if (smb.smbNum[a] ==9){
                        if (numIcon_==6){
                            ratio=size_[numIcon_]/smb.anim[a].height+0.35;
                        }else if (numIcon_==7){
                            ratio=size_[numIcon_]/smb.anim[a].height+0.45;
                        }                    
                    }else if (smb.smbNum[a] ==10 || smb.smbNum[a] ==12){
                        if (numIcon_==5){
                            ratio=size_[numIcon_]/smb.anim[a].height-0.03;
                        }else if (numIcon_==6){
                            ratio=size_[numIcon_]/smb.anim[a].height-0.02;
                        }else if (numIcon_==7){
                            ratio=size_[numIcon_]/smb.anim[a].height-0.01;
                        }                    
                    }            
                }
    ...
    function iconAdjustment_(smbNum,image,icon,frame){
            ...
            if (isBrandedMw()) {
                var F = game_.add.sprite(frame.x,frame.y, "smbFrame" + icon +"_" + smbNum);
            }else{
        ...
        if (gameParams.gameOriginalID!=7030){
            if (isBrandedMw()) {
                if (smbNum > 4 && (icon == 7 || icon == 8 || icon == 9)) {
                    offset = 2.5 * (smbNum)
                    image.y.y = image.y - offset;
                }

                var mask_ = new Phaser.Rectangle(0, offset, frame.width, frame.height-offset);
            }else {
                if (smbNum == 7) {
            ...
            if (icon==7 || icon ==8 || icon ==9){
                if (smbNum == 3   ) {
                    ...
                    var mask_ = new Phaser.Rectangle(0, 7, frame.width, frame.height);
                    image.y=image.y+0; 
                }else if (smbNum == 6 ) {
                    var mask_ = new Phaser.Rectangle(0, 10, frame.width, frame.height+15);
                    image.y=image.y-0; 
                }else if (smbNum == 7 ) {
                    var mask_ = new Phaser.Rectangle(0, 7, frame.width, frame.height+30);
                    image.y=image.y+0; 
                }else{
                    var offset=2.5*(smbNum)
                    var mask_ = new Phaser.Rectangle(0,offset,frame.width, frame.height);
                    image.y=image.y-offset;                        
                }
            }else{
                var offset=2.5*(smbNum)
                var mask_ = new Phaser.Rectangle(0,offset,frame.width, frame.height);
