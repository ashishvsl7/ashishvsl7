...
function TotallyTumblingSpin(gameRef, gameClass) {
    ...
    function setReels_(){
        ...
        if ((loginVars.getRegParams().jurisdiction !="es"  && loginVars.getRegParams().jurisdiction !="se" && loginVars.getRegParams().jurisdiction !="co" )|| now-timeSpin_>2500 ){
            setReelsNow_();
        }else{
    ...
    function resetSmb_(resumeJson,i,a){
        ...
        if(resumeJson[i].smb[a]!=undefined) {
            if (resumeJson[i].smb[a].smb == 13 && gameParams.gameID=="7017") {
                obj.type = "anim";
                obj.loop = true;
                obj.callBack = null;
            } else {
