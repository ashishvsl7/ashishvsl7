/**
 * KiS Framework, Created by Fabry on 22/01/2020.
 */
function MegaWayTumblingWinjack(gameRef, gameClass) {
    ...
    //x20, 50x,  x100
    var winAnim = {
        //"winClasses_":    [1,   9,  19,  40, 80, 199, 400, 1000, 1500],
        "winClasses_":    [0,   .5,   1,   2,   4, 10,  20,  50, 70],
        "winSound_":    [0,   0,  2,  3, 4, 5, 6, 7, 8],
        "winTxtDuration": [1,  1, 1, 2,   3, 4, 6,   7,    8],
    ...
    function showBonusWin_(amt,amtCash){
        ...
        coinWinTxt_.y=(getOrientation()=="portrait")?360: 380;
        coinWinTxt_.scale.x=4;
        coinWinTxt_.scale.y=4;
        return 1500;
    }
    ...
    function prepareWinnings_(winSmb) {
        ...
        if (lineWinAmt_==0 && gameClass.hasScatter()){
            scatterOnly_=true;
        }else{
            scatterOnly_=false;
        }
        ...
        if (fsType_=="tumble" || fsType_=="convert" ){
            ...
            console.log("--tumbling? "+ tumbling_);
            //bSkip_=true;
    ...
    function showLine_() {
        ...
        if (tumbling_)winEventManager_.addEvent(tumble_,(gameClass.hasScatter())?1200:900);
        if (tumbling_)winEventManager_.addEvent(fakeEvent,500);
        if (tumbling_)winEventManager_.addEvent(restoreIcons,700);
        ...
        //     winEventManager_.addEvent(unlock_,3000);
        // }else{
    ...
    function showCentralWin_(params){
        if (gameClass_.getHarpPlayed()==true) {
            ...
            if (freeSpinsManager_.getIsInFs() == true) {
                coinWinTxt_.y = (getOrientation() == "portrait") ? 330 : 630;
            }
                    ...
                    if(freeSpinsManager_.getIsInFs()==false) {
                        if (index>1) {
                           ...
                           // winManager_.resetWinAmt();
                            ...
                            //}
                            //if (spinManager_.getSpinResp().win.scatter==undefined|| spinManager_.getSpinResp().win.scatter[0]==undefined||spinManager_.getSpinResp().win.scatter[0][12]==undefined)gameClass_.updateBalance( tumblingWin_);
                            coinWinTxt_.setText("");
                        }
                    }else{
        ...
        if (gameClass_.getHarpPlayed()==true) {
            ...
            if (last) {
                ...
                if (reset_ == false) tw_.push(TweenMax.to(obj, winAnim.winTxtDuration[index] - 1, {
                    ...
                    onCompleteParams:[winAnim.centralWinDur[index] - 1],
                    ease: Power0.easeOut
                }));

                winAmtManager_.showIstantUpdateWin(tumblingWin, "Total WIN");
               // if (spinManager_.getSpinResp().win.scatter==undefined|| spinManager_.getSpinResp().win.scatter[0]==undefined||spinManager_.getSpinResp().win.scatter[0][12]==undefined)gameClass_.updateBalance( tumblingWin_);gameClass_.updateBalance(tumblingWin_);
    ...
    function finalizeCentralWin_(tOut){
        ...
        tw3=setTimeout(gameClass_.increaseVolumeBG,Math.max(Math.min(tOut*1000,5000),2500));
        setTimeout(resetCentralWin_,Math.max(Math.min(tOut*1000,5000),2500));
        coinWinTxt_.setText("");
    ...
    function delayedAnimation_(smb,a){
        ...
        if (spinManager_.getAnimationMap(smb.reel[a],smb.icon[a]).animations==null || ReelConfig.keepShowingAnimation==true) //avoid to add the same animation more then once
        {
            ...
            if (F!=null){
                ...
                if (smb.smbNum[a]==12) {
                    gameClass_.logger("reel " + smb.reel[a] + " anim smb " + smb.icon[a] + " num12 MC "+ smb.anim[a].smbNum )
                    //leaves + 1 animation
                    ...
                    var lf= game_.add.sprite(smb.anim[a].x, smb.anim[a].y -20, "anchorIn_1" );
                    lf.alpha=1;
                    lf.anchor.set(.5,.5);
                    lf.animations.add("anim");
                    lf.animations.play("anim",24,false,true);
                    lf.scale.x=.6;
                    lf.scale.y=.6;
                    displayManager_.getGroup("beans").add(lf);

                    TweenMax.to(lf,.5,{delay:.7,alpha:0});
                    TweenMax.to(lf,.8,{delay:.5,y:smb.anim[a].y -100});
    ...
    function resetCentralWin_(now){
        gameClass_.logger("resetCentralWin " );
    ...
    function changeCentralWinStage_(flip){
        ...
        if (stage_==1) {
            ...
            gameClass_.logger("scale first " + winAnim.scaleFactor[6 + stage_]);
            // coinWinTxt_.scale.x = winAnim.scaleFactor[6 + stage_];
