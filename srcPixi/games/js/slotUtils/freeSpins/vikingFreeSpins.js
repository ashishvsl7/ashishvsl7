/**
 * KiS Framework, Created by Fabry on 22/11/2018.
 */
function VikingFreeSpins(gameRef,gameClass){
    var game_=gameRef;
    ...
    function parse_(json){
        ...
        if (json.freeSpin!=null){
            //inside FS
            if (json.freeSpin.type!="tumble"){
                freeSpinNum_=json.freeSpin.num;
            }else{
                freeSpinNum_=json.freeSpin.num-1;
            }
    ...
    function showFsTotWonPanel_(){
        ...
        if (freeRoundsManager_.getIsInFr()==false){
            displayManager_.getGroup("logo").logo.children[0].visible=false;
            TweenMax.to(displayManager_.getGroup("logo"),.3,{y:0,ease:Bounce.easeOut,onComplete:gameClass_.logoIn});
        }else{
    ...
    function hideFs_(){
        ...
        TweenMax.to(displayManager_.getGroup("logo"),.3,{delay:.5,y:0,onComplete:gameClass_.logoIn});
        displayManager_.getGroup("logo").logo.children[0].visible=false;
        isInFs_ = false;
    }
