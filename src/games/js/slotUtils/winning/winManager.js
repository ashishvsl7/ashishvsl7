function WinManager(gameRef, gameClass) {
    ...
    function evaluate_(reels) {
        ...
        if (gameParams.gameOriginalID !="7008" && gameParams.gameOriginalID !="7017" && gameParams.gameOriginalID !="7019"&& gameParams.gameOriginalID !="7029" && gameParams.gameOriginalID !="7023" && gameParams.gameOriginalID !="7012"  && gameParams.gameOriginalID !="7014" && gameParams.gameOriginalID !="7024"&& gameParams.gameOriginalID !="7025"&& gameParams.gameOriginalID !="7026"&& gameParams.gameOriginalID !="7027"&& gameParams.gameOriginalID !="7127"&&gameParams.gameOriginalID !="7227"&& gameParams.gameOriginalID !="7028" && gameParams.gameOriginalID !="7016" && !isBrandedMw() ) {//win combination games don't have valid lines
        ...
        if (gameParams.gameOriginalID !="7015" && spinManager_.getFsResp()!=null && spinManager_.getFsResp().won>0 && spinManager_.getFsResp().type!="tumble" && spinManager_.getFsResp().type!="convert" && spinManager_.getFsResp().type!="respin"){
            if ((gameParams.gameOriginalID =="7019" ||gameParams.gameOriginalID =="7029" || gameParams.gameOriginalID =="7025") && freeSpinsManager_.getIsInFs() )return ret;//MWJ ,rwmw, fsWon hadled separately for additional FS
            ...
            if (gameParams.gameOriginalID =="7029"){//SBB
                ...
                freeSpinsManager_.fsWon(spinManager_.getFsResp().won);
            }
        ...
        for (var i in winSmb_) {
            if (winSmb_[i] != undefined) {
                for (var b in winSmb_[i].anim){
                    if(winSmb_[i].anim[b]) {
                        winSmb_[i].anim[b].animations.stop();
                ...
                for (var s in winSmbTemp[i]) {
                    for (var b in winSmbTemp[i][s].anim) {
                        if (winSmbTemp[i][s].anim[b]) {
                            winSmbTemp[i][s].anim[b].animations.stop();
    ...
    function getTumblingWinningSymbols_() {
        ...
        for (i = 0; i < reels.length; i++) {
            if (spinManager_.getSpinReelResp().reel[i]!= undefined) {
                var icons = (gameParams.gameOriginalID == "7016" || gameParams.gameOriginalID == "7019" || gameParams.gameOriginalID == "7029" || gameParams.gameOriginalID == "7023" || isBrandedMw()) ? spinManager_.getSpinReelResp().reel[i].smb.length : ReelConfig.numIcons;
                for (var a = 0; a < icons; a++) {
                    if (spinManager_.getSpinReelResp().reel[i].smb[a] != undefined) {


                        var aa = (gameParams.gameOriginalID == "7016" || gameParams.gameOriginalID == "7019" || gameParams.gameOriginalID == "7029" || gameParams.gameOriginalID == "7023"  || gameParams.gameOriginalID == "7025" || gameParams.gameOriginalID == "7026"  || gameParams.gameOriginalID == "7027"   || gameParams.gameOriginalID == "7127"   || gameParams.gameOriginalID == "7227"  || gameParams.gameOriginalID == "7028" || isBrandedMw()) ? (spinManager_.getSpinReelResp().reel[i].smb.length - 1) - a + ReelConfig.numIconsOnBottom : a;
                        ...
                        if (spinManager_.getSpinReelResp().reel[i].smb[a].special.indexOf("tumble") >= 0)
                            ...
                            if (spinManager_.getSpinReelResp().reel[i].smb[a].special == "tumble" && spinManager_.getSpinReelResp().reel[i].smb[a].smb != 14) {//todo implement better smb 14 check
                                ...
                                obj.tumble.push(true);
                                obj.pos.push(reels[i].getPos(aa));
                                var smb = (gameParams.gameOriginalID == "7016" || gameParams.gameOriginalID == "7019" || gameParams.gameOriginalID == "7029" || gameParams.gameOriginalID == "7023" || gameParams.gameOriginalID == "7025" || gameParams.gameOriginalID == "7026" || gameParams.gameOriginalID == "7027" || gameParams.gameOriginalID == "7107" || gameParams.gameOriginalID == "7227"|| gameParams.gameOriginalID == "7028" || isBrandedMw()) ? reels[i].getArr()[aa] : reels[i].getSymbol(aa);
                                smb.attributes = spinManager_.getSpinReelResp().reel[i].smb[a];
                                obj.split.push(spinManager_.getSpinReelResp().reel[i].smb[a].split != undefined) ? spinManager_.getSpinReelResp().reel[i].smb[a].split : false;
                                obj.splitOrigin.push(spinManager_.getSpinReelResp().reel[i].smb[a].splitOrigin != undefined) ? spinManager_.getSpinReelResp().reel[i].smb[a].splitOrigin : -1;
                                obj.simbolReference.push(smb);
