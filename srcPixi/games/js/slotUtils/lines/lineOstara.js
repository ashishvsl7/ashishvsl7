 ...
 */
function LineOstara(gameRef, gameClass) {
/*    var gameClass_ = gameClass;
    ...
    var winAnim= {
        ...
        "scaleFactor":    [1.6,  1.8  ,2,   2.2, 2.4, 2.6,  3,   3.5,  4,  5],
    ...
    function showLine_() {
        ...
        if (freeSpinsManager_.getIsInFs()==false) {
            ...
            if(winManager_.getScatterWinAmt()[0]!=undefined && winManager_.getScatterWinAmt()[0][10].amt>0){
                ...
                winEventManager_.addEvent(addScatterWin_,0);
                var t = winAnim.winTxtDuration[findWinRange_(amt)] * 1000;
                winEventManager_.addEvent(fakeEvent,3700);
            }
            ...
            winEventManager_.addEvent(updatePrize_, 0, [winManager_.getLineWinAmt(),totalDuration_/1000]);
    ...
    function addScatterWin_(){
        var amt;
        if (balanceManager_.getShowIncredits() == false) {
            amt=   winManager_.getScatterWinAmt()[0][10].amt;
        } else {
            amt=   winManager_.getScatterWinAmt()[0][10].amt / balanceManager_.getCoinValue() * lineManager_.getLinesNumber();
        }
    ...
    function updateBalance_(){
        reset_=true;
        if (freeSpinsManager_.getIsInFs()==false || freeSpinsManager_.getFsNumber()==-999 || freeSpinsManager_.getFsWon()==true) {
    ...
    function randomSparkle_(smb){
        //spread sparkles from the center of the iconwhen the lines go through
        for (var a=0;a< Util.getRandomInt(10,20);a++){
            var s = game_.add.sprite(smb.x +Util.getRandomInt(-45,45) , smb.y+Util.getRandomInt(-35,35),"sparkle");
            s.anchor.set(.5, .5);
            s.animations.add("anim");
            s.scale.x=Util.getRandomInt(6,12)/10;
            s.scale.y=s.scale.x;
            s.angle=(Util.getRandomInt(0,90));
            coinWinGroup_.add(s);
            s.animations.play("anim", 12, false,false);
            TweenMax.to(s,1,{y:s.y+80,onComplete:kill_,onCompleteParams:[s]})
        }
    }
    ...
    function showBonusCentralWinGreetings_(amt){
        ...
        for (var a in stages_){
            if (Number(amt)>=stages_[a]){
                time =winAnimB.winTxtDuration[findWinRangeB_(amt)] * 500;
                index=a;
            }
        }
        if (index>=0){
