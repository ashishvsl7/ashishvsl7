...
function Spin(gameRef, gameClass) {
    ...
    function setReels_(){
        ...
        var waitWildAnim=(gameClass_.waitWildAnim!=null)?gameClass_.waitWildAnim():false;
        if (!waitWildAnim){
        	responseReceived=true;
        }else{
            //first show the wild fight or move wilds while reels
            if(gameClass_.spinFightAnim!=null)gameClass_.spinFightAnim();
        }
    ...
    function startSpin_() {
        ...
        for (var i = 0; i < ReelConfig.numReels; i++) {
            //
            // reel_[i].replaceIcon("lowerIcon0",  reel_[i].getIcon("lowerIcon0").smbNum);
            // reel_[i].replaceIcon("lowerIcon1", reel_[i].getIcon("lowerIcon1").smbNum);
            // reel_[i].replaceIcon("lowerIcon2", reel_[i].getIcon("lowerIcon2").smbNum);
            //
            // reel_[i].replaceIcon("upperIcon0",  reel_[i].getIcon("upperIcon0").smbNum);
            // reel_[i].replaceIcon("upperIcon1", reel_[i].getIcon("upperIcon1").smbNum);
            // reel_[i].replaceIcon("upperIcon2", reel_[i].getIcon("upperIcon2").smbNum);
    ...
    function upperSmb_(i) {
        ...
        for (var j in animationMap_[i]) {
            //removeAllChildren(reel_[i].getAllIcons());
            animationMap_[i][j]={};
        }
    ...
    function onCompleteFinal(i) {
        ...
        if (reelToStop_!="" && ReelConfig.stopTurboReel==true){
            ...
            console.log("stop reelSoundFast_ "+reelToStop_)
            reelToStop_="";
        }
        if (iconSound[i]==true && !isStopped_){
            console.log("play reelSoundFast_"+i)
    ...
    var me = {
        ...
        setResponseReceived:function(){
            //set by wild fighting class when the fight between wild is completed and game can finally end the spin animation
            responseReceived=true;
        }
