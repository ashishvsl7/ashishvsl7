 ...
 */
function MegaWaySplitTumblingWin(gameRef, gameClass) {
    ...
    var winAnim = {
        ...
        "winTxtDuration": [0.2,  0.5, 1, 1,   1.5, 3, 4,   5,    6],
    ...
    function prepareWinnings_(winSmb) {
        ...
        addFs=(spinManager_.getSpinResp()!=undefined && spinManager_.getSpinResp().freeSpin!=undefined)?spinManager_.getSpinResp().freeSpin.object.wonAdditionalSpins:undefined;
        if (addFs!=undefined && freeSpinsManager_.getIsInFs()){
            if (spinManager_.getSpinResp().win.scatter!=undefined && spinManager_.getSpinResp().win.scatter.scatters!=undefined) {
                for (var a in spinManager_.getSpinResp().win.scatter.scatters){
                    if ( spinManager_.getSpinResp().win.scatter.scatters[a].smbID==13){
                        addFs=spinManager_.getSpinResp().win.scatter.scatters[a].num;
                        fs=  Number(displayManager_.getText("fsValue").text)+addFs;
                    }
                }
            }
        }
    ...
    function showLine_() {
        ...
        if (hasBeenCapped_ == 0) {
            ...
            if (tumbling_) winEventManager_.addEvent(tumble_, 600);
            winEventManager_.addEvent(fakeEvent, 400, spinManager_.tumbleDone)
    ...
    function delayedAnimation_(smb,a){
        ...
        if (smb.smbNum[a]!=11) {
            ...
            if (spinManager_.getReels()[smb.reel[a]].getIconSize(smb.icon[a]) >= 3 && smb.smbNum[a] < 8) {
                var ratio= 1- spinManager_.getReels()[smb.reel[a]].getIconSize(smb.icon[a]) *.035;
                smb.anim[a].scale.x = Math.min(ratio, 1);
                smb.anim[a].scale.y = Math.min(ratio, 1);
            }

            if (game.cache.checkImageKey("smbFrame_" + smb.smbNum[a] + "_2")) {
                var F = iconAdjustment_(spinManager_.getSpinReelResp().reel[smb.reel[a]].smb.length, smb.anim[a], smb.smbNum[a], spinManager_.getFrameMW(smb.reel[a], smb.icon[a]), spinManager_.getReels()[smb.reel[a]].getIconSize(smb.icon[a]), spinManager_.getReels()[smb.reel[a]].getIconHeight(smb.icon[a]));
                smb.anim[a].F = F;
                smb.anim[a].newH=smb.anim[a].height;
                smb.anim[a].newW=smb.anim[a].width;
            } else {
                iconAdjustmentNoFrame_(spinManager_.getSpinReelResp().reel[smb.reel[a]].smb.length, smb.anim[a], smb.smbNum[a], spinManager_.getReels()[smb.reel[a]].getIconSize(smb.icon[a]), spinManager_.getReels()[smb.reel[a]].getIconHeight(smb.icon[a]));
            }
            ...
            smb.anim[a].WH = spinManager_.getWormHoleMW(smb.reel[a], smb.icon[a]);
            if (smb.smbNum[a]==10){
                if(smb.anim[a].BG!=undefined)smb.anim[a].BG.visible=false;
                if(smb.anim[a].F!=undefined)smb.anim[a].F.visible=false;
            }else if  (smb.smbNum[a]==12){
                soundManager_.playSound("fuel");
            }else if  (smb.smbNum[a]==13){
                soundManager_.playSound("superFuel");
            }
            ...
            delayedWildReelAnimation_(smb.reel[a]);
        }
    ...
    function iconAdjustment_(smbNum,image,icon,frame,iconSize, iconHeight){
        //create animation frame based on icon frame pos, this will be added later to the reel group
        var F = game_.add.sprite(frame.x,frame.y, "smbFrame_" + icon  + "_" +iconSize);
        F.scale.x=1;
        F.scale.y=1;
        F.anchor.set(.5,.5);
        ...
        if (icon==8){
            ...
            if (iconSize==7){
                var mask_ = new Phaser.Rectangle(0, 70,100,iconHeight);
                image.height=image.height-20;
            }else if (iconSize==6){
                var mask_ = new Phaser.Rectangle(0, 65, 100,iconHeight);
                image.height=image.height-20;
            }else if (iconSize==5){
                var mask_ = new Phaser.Rectangle(0, 60, 100,iconHeight);//ok
                image.height=image.height-20;
            }else if (iconSize==4){
                var mask_ = new Phaser.Rectangle(0, 60, 100,iconHeight);//ok
                image.height=image.height-20;
            }else if (iconSize==3){
                var mask_ = new Phaser.Rectangle(0, 50, 100,iconHeight); //ok
            ...
            if (iconSize==7){
                var mask_ = new Phaser.Rectangle(0, 80, 100,iconHeight);
                image.height=image.height-20;
            }else if (iconSize==6){
                var mask_ = new Phaser.Rectangle(0, 73, 100,iconHeight);
                image.height=image.height-20;
            }else if (iconSize==5){
                var mask_ = new Phaser.Rectangle(0, 70, 100,iconHeight);
                image.height=image.height-20;
            }else if (iconSize==4){
                var mask_ = new Phaser.Rectangle(0, 65, 100,iconHeight); //ok
                image.height=image.height-20;
            }else if (iconSize==3){
                var mask_ = new Phaser.Rectangle(0, 40, 100,iconHeight); //ok
                image.height=image.height-10;
            }
        }else if (icon==12){

            if (iconSize==7){
                var mask_ = new Phaser.Rectangle(0, 100, 100,iconHeight);
                image.height=image.height-20;
            }else if (iconSize==6){
                var mask_ = new Phaser.Rectangle(0, 90, 100,iconHeight);
                image.height=image.height-20;
            }else if (iconSize==5){
                var mask_ = new Phaser.Rectangle(0, 80, 100,iconHeight);
                image.height=image.height-20;
            }else if (iconSize==4){
                var mask_ = new Phaser.Rectangle(0, 70, 100,iconHeight); //ok
                image.height=image.height-20;
            }else if (iconSize==3){
                var mask_ = new Phaser.Rectangle(0, 40, 100,iconHeight); //ok
                image.height=image.height-10;
            ...
            if (iconSize==7){
                var mask_ = new Phaser.Rectangle(0, 57, 100,iconHeight);
                image.height=image.height-20;
            }else if (iconSize==6){
                var mask_ = new Phaser.Rectangle(0, 50, 100,iconHeight);
                image.height=image.height-20;
            }else if (iconSize==5){
                var mask_ = new Phaser.Rectangle(0, 50, 100,iconHeight);
                image.height=image.height-20;
                ...
                var mask_ = new Phaser.Rectangle(0, 20, 100,iconHeight); //ok
                image.height=image.height-10;
    ...
    function startFallsDown_(ob){
        ...
        TweenMax.to(ob,  Util.getRandomInt(180, 300) /100, {
            y: ob.y+500,
    ...
    function showBonusCentralWinGreetings_(amt){
        ...
        //displayManager_.getText("lineWin").y=332;
        displayManager_.getText("lineWin")._fontSize=50;
    ...
    function startNewSmallObject_(){
        ...
        TweenMax.to(ob, .1 + Util.getRandomInt(120, 200) / 100, {
            alpha: 1,
            rotation:Util.getRandomInt(0, 90),
            y: Util.getRandomInt(top, 490),
            x: ob.xVar, onComplete: startFallsDown_,  onCompleteParams: [ob],ease:Circ.easeOut
        });
    ...
    function startNewObject_(par){
        ...
        TweenMax.to(ob, .1 +  Util.getRandomInt(220, 300) / 100, {
            alpha: 1,
            rotation:Util.getRandomInt(0, 90),
