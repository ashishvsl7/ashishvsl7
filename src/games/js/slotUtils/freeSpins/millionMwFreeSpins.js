 ...
 */
function MillionMwFreeSpins(gameRef,gameClass){
    var game_=gameRef;
    ...
    function showFsTotWonPanel_(){
        ...
        if (isInFs_) {
            ...
            TweenMax.to(displayManager_.getGroup("fsWonPanel"),.4,{y: (getOrientation()=="portrait")?200* gameClass.getRatio():0,ease:Bounce.easeOut,onComplete:hideTotPanel_});
        }else{
            ...
            TweenMax.to(displayManager_.getGroup("fsWonPanel"), .4, {
                y: (getOrientation()=="portrait")?200* gameClass.getRatio():0,
    ...
    function delayedHidePanel_(){
        if (gameClass_.getLineCompletion()) {
            ...
            setTimeout(delayedHidePanel_,1000);
        }
    }
    ...
    function stopbg(){
        for ( var i =1;i<8;i++){
            if(soundManager_.getBGSound("bgSlot"+i)!=undefined)soundManager_.getBGSound("bgSlot"+i).fadeOutBgSound(200);
        }
    }
    ...
    function showFsPanel_(){
        ...
        gameClass_.decreaseVolumeBG(3000);

        setTimeout(realShowPanel_,500)
    }

    function realShowPanel_() {
        //mult_=1;
    ...
    function toggleBackFs_(cap){
        if (gameClass_.getLineCompletion()) {
            if (capAmt_==0) {
                ...
                isInFs_ = true;
                displayManager_.getText("fsCap").setText("");
                displayManager_.getGroup("fsWonPanel").fsHeading.children[0].visible=true;
                ...
                if (!isInFs_)displayManager_.getGroup("fsWonPanel").fsHeading.children[0].visible=false;
            ...
            setTimeout(toggleBackFs_,500,cap);
        }
    }
