...
function AllLineWinningsFirstLoop(gameRef, gameClass) {
    ...
    function initClass_() {
        ...
        //bg
        centralWinMsgBg[1]=displayManager_.getGroup("centralWin").bg_3;
        centralWinMsgBg[2]=displayManager_.getGroup("centralWin").bg_3;
        centralWinMsgBg[3]=displayManager_.getGroup("centralWin").bg_3;
        centralWinMsgBg[4]=displayManager_.getGroup("centralWin").bg_4;
        //text
        centralWinMsgTxt[1]=displayManager_.getGroup("centralWin").bigWin;
        centralWinMsgTxt[3]=displayManager_.getGroup("centralWin").epicWin;
        centralWinMsgTxt[2]=displayManager_.getGroup("centralWin").ultraWin;
        centralWinMsgTxt[4]=displayManager_.getGroup("centralWin").jpotWin;
        //pos
        centralWinMsgPos_[1]=displayManager_.getGroup("centralWin").bigWin.y;
        centralWinMsgPos_[3]=displayManager_.getGroup("centralWin").epicWin.y;
        centralWinMsgPos_[2]=displayManager_.getGroup("centralWin").ultraWin.y;
        centralWinMsgPos_[4]=displayManager_.getGroup("centralWin").jpotWin.y;
        //ray
        centralWinRay_=displayManager_.getGroup("centralWin").winRays;
        //popup
        centralWinPopUp_=displayManager_.getGroup("centralWin").bigWinBg;
        //funtain
    ...
    function stopIncSound_() {
        // todo p3 soundManager_.stopSound(winAnim.accuSnd);
    ...
    function calcSingleLinePoints_(line, winSmb) {
        ...
        for (var w in winSmb.pos) {
            ...
            pp = new Point(lineConfig.line.x_origin, 0);
            if (w == 0) {   //first symbol
                real = new Point(winSmb.pos[w].x + pp.x, winSmb.pos[w].y + smbH_ / 2 + pp.y);
    ...
    function resetPrevSounds_(){
        for (var i=0;i<9;i++){
            // todo p3 soundManager_.stopSound("winBg_"+i);
            soundManager_.stopSound("winBg_"+i);
        }
        for (var i=6;i<9;i++) {
            // todo p3 soundManager_.stopSound("winBg_S_" + i);
    ...
    function showLine_() {
        ...
        // todo p3 soundManager_.playSound("winBg_"+ winAnim.winSound_[index]);   //total bg sound

        //decrease music bg for winnings
        //todo p3 gameClass_.decreaseVolumeBG(me.getTotalDuration());
        ...
        // todo p3 soundManager_.playSound("smbWin_"+iMaxSmb);
    ...
    function drawLine_(params) {
        ...
        if (coinsIconPoints_[line].length > 0) {
            ...
            // todo p3 if (snd && (stage_<=0  || ReelConfig.muteDuringWins!=true))soundManager_.playSound("smbWin_"+smb);
    ...
    function updatePrize_(){
        ...
        bCompleted_=true;
        // todo p3 soundManager_.playSound("smbFsWin");
    ...
    function idle_(force) {
        ...
        wins=winManager_.getAllWinsSortedByAmt();
        winEventManager_.clearEvt();
        coinWinTxt_.scaleX = scaleStart_;
        coinWinTxt_.scaleY =scaleStart_;
        for (var a in wins) {
            if (wins[a] != undefined) {
                if (wins[a].winAmount > 0) {
                    winEventManager_.addEvent(changeAllHL_, 200, [.35]);
    ...
    function hlSmb(par) {
        ...
        a.alpha = 1;
        TweenMax.to(a, .2, {scaleX: 1.1, scaleY: 1.1, onComplete: updateAfterWinScale_, onCompleteParams: [a]});
    ...
    function startCoinsSound_(){
        // todo p3 soundManager_.playSound("coins");
    ...
    function updateBalance_(){
        ...
        // todo p3 soundManager_.playSound("line");
    }
    ...
    function updateAfterWinScale_(a){
        if (a == null|| a==undefined)return;
        if (a.scale == null || a.scale==undefined)return;
        TweenMax.to(a,.2,{scaleX:1,scaleY:1});//todo P3 uses scaleX
    }
    ...
    function playBiggerIconSound_(iconN){
        // todo p3 soundManager_.playSound("smbWin_"+iconN);
    ...
    function delayedAnimation_(smb,a){
        ...
        if (spinManager_.getAnimationMap(smb.reel[a],smb.icon[a]).animations==null || ReelConfig.keepShowingAnimation==true) //avoid to add the same animation more then once
        {
            ...
            add=true;
            if (smb.smbNum[a]!=6) {
                smb.anim[a] = spriteManager_.addSprite(smb.pos[a].x + ReelConfig.icon.width / 2, smb.pos[a].y + ReelConfig.icon.height / 2, "anim" + smb.smbNum[a], null, null, .5, .5, 1, 1);
                smb.anim[a].animations = animationManager_.create(smb.anim[a], "anim", 24, false, false, restoreIcon_, [spinManager_.getAnimationMap(smb.reel[a], smb.icon[a])]);
                smb.anim[a].spine=false;
            }else{
                smb.anim[a]=game_.add.spine(smb.pos[a].x + ReelConfig.icon.width / 2, smb.pos[a].y + ReelConfig.icon.height / 2, "anim" + smb.smbNum[a], 'anim', true);
                smb.anim[a].setScale(0.195);
                smb.anim[a].spine=true;
                //setTimeout(pause,200,smb.anim[a]);
        ...
        if (smb.anim[a].scaleX <=1) {
            if (smb.smbNum[a]!=6) {
                if (add == true || lineConfig.line.multipleAnimation[smb.smbNum[a]] == true)
                    animationManager_.play(smb.anim[a], "anim", 24, false, false, restoreIcon_, [spinManager_.getAnimationMap(smb.reel[a], smb.icon[a])]);
                else
    ...
    function pause(obj){
        //debugger;
        obj.state.timeScale = 0; // set to 1 to resume
        setTimeout(resume,200,obj)
    }

    function resume(obj){
        obj.state.timeScale = 1; // set to 1 to resume
        setTimeout(pause,200,obj)
    }
    ...
    function restoreIcon_(par){
        ...
        TweenMax.to(smb, .1, {scaleX: 1, scaleY: 1});
    }
    ...
    function showBonusCentralWinGreetings_(amt){
        ...
        if (index>0){
            if (amt<39999) {
                ...
                ImmediateCentralWinStage_(amt);
                // todo p3 soundManager_.playSound("winBgS_"+(Number(stage_) + 6));
                ...
                stage_=4;
                ImmediateCentralWinStage_(amt);
                // todo p3 soundManager_.playSound("winBg_9");
    ...
    function resetCentralWin_(now){
        ...
        //todo p3  soundManager_.stopSound("winBg");
        ...
        for (var a=1; a<=centralWinMsgBg.length-1;a++) {
            TweenMax.to(centralWinPopUp_,.5,{scaleX:1.1,scaleY:1.1,ease: Sine.easeInOut});
            if (now==true){
                removeCW_(a);
                TweenMax.to(centralWinMsgTxt[a],.5,{scaleX:1.2,scaleY:1.2,ease: Sine.easeInOut});
            }else{
                TweenMax.to(centralWinMsgTxt[a],.5,{scaleX:1.2,scaleY:1.2,ease: Sine.easeInOut,onComplete:removeCW_,onCompleteParams:[a]});
            }
        }
        funtainEventManager_.clearEvt();
        // for (a in objThrown_){
    ...
    function changeCentralWinStage_(flip){
        ...
        if (stage_==1) {
            ...
            coinWinTxt_.scaleX = winAnim.scaleFactor[6 + stage_];
            coinWinTxt_.scaleY = winAnim.scaleFactor[6 + stage_];
            centralWinPopUp_.visible = false;
            if (stage_ < 4) centralWinPopUp_.visible = true;
            centralWinPopUp_.scaleX = 1;
            centralWinPopUp_.scaleY = 1;
            centralWinMsgTxt[stage_].scaleX = 1;
            centralWinMsgTxt[stage_].scaleY = 1;
            ...
            tw2 =TweenMax.to(centralWinMsgTxt[stage_],.5, {
                scaleX: 1.2,
                scaleY: 1.2,
                ease: Sine.easeInOut,
    ...
    function ImmediateCentralWinStage_(amt){
        ...
        coinWinTxt_.scaleX = 3;
        coinWinTxt_.scaleY = 3;
        centralWinPopUp_.visible = false;
        if (stage_ < 4) centralWinPopUp_.visible = true;
        centralWinPopUp_.scaleX = 1;
        centralWinPopUp_.scaleY = 1;
    ...
    function change_(s){
        ...
        centralWinMsgTxt[s.stage].scaleX=1;
        centralWinMsgTxt[s.stage].scaleY=1;

        coinWinTxt_.y=550;
        console.log("scale "+winAnim.scaleFactor[6+nextS]);
        coinWinTxt_.scaleX= winAnim.scaleFactor[6+nextS];
        coinWinTxt_.scaleY= winAnim.scaleFactor[6+nextS];
        centralWinPopUp_.visible = false;
        if (nextS < 4) centralWinPopUp_.visible = true;
        centralWinPopUp_.scaleX = 1;
        centralWinPopUp_.scaleY = 1;
        centralWinMsgTxt[nextS].scaleX = 1;
        centralWinMsgTxt[nextS].scaleY = 1;
        ...
        centralWinMsgTxt[nextS].scaleY=0;
        centralWinMsgTxt[nextS].scaleX=0;
        centralWinMsgTxt[nextS].stage=nextS;
        tw2 =TweenMax.to(centralWinMsgTxt[nextS],.4,{scaleX:1.2,scaleY:1.2,  onComplete: scaleBack2_,onCompleteParams: [centralWinMsgTxt[nextS]] });
    ...
    function scaleBack2_(s){
        ...
        if (bookChange_>0){
            this.visible=false;
            this.alpha=false;
            tw2=TweenMax.to(s,.4,{scaleX:.7,scaleY:.7,ease: Sine.easeInOut,onComplete: change_,onCompleteParams: [s]});
        }else{
            tw2=TweenMax.to(s,.4,{scaleX:1,scaleY:1,ease: Sine.easeInOut,onComplete: scaleAgain_,onCompleteParams: [s]});
        }
    ...
    function startNewSmallObject_(){
        ...
        ob.scaleX = Util.getRandomInt(100,80)/100;
        ob.scaleY = ob.scaleX;
        objThrown_.push(ob);
    ...
    function startNewObject_(){
        ...
        ob.animations.play("anim", Util.getRandomInt(24, 34), true, false, null, null);//todo parametric loop
        ob.scaleX = Util.getRandomInt(displayManager.groups.centralWin.minScale,displayManager.groups.centralWin.minScale, displayManager.groups.centralWin.minScale,displayManager.groups.centralWin.maxScale)/100;
        ob.scaleY = Util.getRandomInt(displayManager.groups.centralWin.minScale,displayManager.groups.centralWin.minScale, displayManager.groups.centralWin.minScale,displayManager.groups.centralWin.maxScale)/100;
        objThrown_.push(ob);
